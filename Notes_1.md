...
## Q19: `.KptRevisionMetadata` not found — Must `pull` before `push`

### Error

```
Error: ".KptRevisionMetadata" not found in PackageRevisionResources 'porch-demo/blueprints-756b6fc52d48ff5ef548d8def511f9340e751ed3'
```

This happens when running `kpt alpha rpkg push` (or `porchctl rpkg push`) without first pulling the package.

### Root Cause

When you create a package with `init` or `copy`, Porch creates it **server-side** and generates hidden lifecycle metadata (`.KptRevisionMetadata`) as part of the package resources. Your local files don't have this metadata. The `push` command requires it to be present in the directory you're pushing.

### The Fix — Always `pull` to an empty directory, edit, then `push`

The correct workflow is:
1. **`pull`** the server-side package into an **empty local directory** (pull refuses to write to an existing directory)
2. **Edit** the pulled files in-place (add, remove, or modify files)
3. **`push`** the same directory back

**Important:** `pull` requires the target directory to **not exist**. If it already exists, you must delete it first.

**Correct `create-blueprint` workflow (verified working):**
```bash
...
# Delete existing directory (pull requires empty target)
rm -rf "$HERE/work/blueprints/network-function/"

# Pull to get the server-generated metadata
kpt alpha rpkg pull "$BLUEPRINT" "$HERE/work/blueprints/network-function/" --namespace=porch-demo

# Edit the pulled files as needed:
#   - Add your deployment.yaml, service.yaml, etc.
#   - Remove files you don't want (e.g., package-context.yaml)
#   - The Kptfile and hidden metadata are already there from the pull

# Push the modified folder back
...
- **Pushing a directory that was never pulled** — your local files won't have `.KptRevisionMetadata`.
- **Copying the `Kptfile` manually** from a pulled directory to your local files — the `Kptfile` alone doesn't contain the metadata; it's stored as a hidden resource key in the package.
- **Pulling into an existing directory** — `pull` will error with `directory already exists`.

### Key Rule

Every time a package is created server-side (`init`, `copy`), the workflow is: **delete local dir → pull → edit in-place → push**. This applies to both `kpt alpha rpkg` and `porchctl rpkg`.

---

## Q20: What is the `work` directory and how is it created?

If you look at the top of the `environments/porch-demo/start` script, you see these two lines:
```bash
rm --force --recursive "$HERE/work/"

GIT_HOST="..." GIT_USERNAME="..." ... envsubst_dir "$HERE/assets" "$HERE/work"
```

### What is it?
The `work` directory is simply a **temporary, auto-generated folder** that holds the finalized Kubernetes manifests. You should **never manually edit** files in the `work/` directory because the script deletes and recreates it every single time you run `start` (hence the `rm --force --recursive` on line 9).

### How is it created?
It is created using a custom bash function called `envsubst_dir` (which is defined in `library/_functions`). 

Here is exactly how that process works:
1. **The Source (`assets/`):** The `porch-demo/assets/` folder contains "template" YAML files. If you open them, you'll notice they have environment variable placeholders in them, like `${GIT_USERNAME}` or `${GIT_HOST}`. These are not valid Kubernetes YAMLs yet.
2. **The Hydration (`envsubst`):** The script sets a bunch of bash environment variables on line 11 (e.g., `GIT_HOST="http://$HOST:32100"`). It then passes the `assets` folder and the `work` folder to the `envsubst_dir` function.
3. **The Output (`work/`):** The `envsubst_dir` tool loops through every file in `assets/`, replaces all the `${PLACEHOLDERS}` with the actual bash variable values (using the standard Linux `envsubst` utility), and saves the filled-in files into the `work/` folder.

---

## Q21: `init` vs `copy` — Creating new packages vs new revisions

### The Two Commands

Porch has two different commands for creating package revisions:

| Command | When to use | What it does |
|---|---|---|
| `init` | **First time only** — package doesn't exist yet | Creates a brand-new empty package (v1) |
| `copy` | **Every subsequent revision** — package already exists | Clones an existing published revision into a new Draft (v2, v3, ...) |

### Error if you use `init` on an existing package

```
error: `init` cannot create a new revision for package "network-function" that already exists in repo "blueprints"; make subsequent revisions using `copy`
```

This is expected — `init` is a one-time operation per package name per repository.

### Real-World Lifecycle

```
FIRST TIME (new package):
  init → rm dir → pull → edit → push → propose → approve     → Published v1

UPDATE (new revision of existing package):
  copy v1 → rm dir → pull → edit → push → propose → approve  → Published v2
  copy v2 → rm dir → pull → edit → push → propose → approve  → Published v3
  ...and so on
```

**Visually:**
```
v1 (Published) ──copy──→ v2 (Draft) ──rm dir──→ ──pull──→ ──edit──→ ──push──→ ──propose──→ ──approve──→ v2 (Published)
```

### Example: Creating and publishing v1 (first time)

```bash
# 1. Init — creates a new empty package on the server
BLUEPRINT=$(kpt alpha rpkg init network-function --workspace=v1 --repository="$GIT_BLUEPRINTS_REPO" --namespace=porch-demo | cut --delimiter=' ' --fields=1)

# 2. Delete local dir (pull requires empty target)
rm -rf environments/porch-demo/work/blueprints/network-function/

# 3. Pull the server-side draft (brings down .KptRevisionMetadata)
kpt alpha rpkg pull "$BLUEPRINT" environments/porch-demo/work/blueprints/network-function/ --namespace=porch-demo

# 4. Edit the pulled folder: add your files, remove defaults you don't want
#    e.g., rm package-context.yaml, add deployment.yaml

# 5. Push, propose, approve
kpt alpha rpkg push "$BLUEPRINT" environments/porch-demo/work/blueprints/network-function/ --namespace=porch-demo
kpt alpha rpkg propose "$BLUEPRINT" --namespace=porch-demo
kpt alpha rpkg approve "$BLUEPRINT" --namespace=porch-demo
```

### Example: Creating v2 from a published v1

```bash
# 1. Copy the published v1 to create a v2 Draft
NEW_DRAFT=$(kpt alpha rpkg copy <published-v1-package-name> --workspace=v2 --namespace=porch-demo | cut --delimiter=' ' --fields=1)

# 2. Delete local dir (pull requires empty target)
rm -rf environments/porch-demo/work/blueprints/network-function/

# 3. Pull the new Draft (brings down .KptRevisionMetadata — see Q19)
kpt alpha rpkg pull "$NEW_DRAFT" environments/porch-demo/work/blueprints/network-function/ --namespace=porch-demo

# 4. Edit the pulled folder: make your changes

# 5. Push, propose, approve
kpt alpha rpkg push "$NEW_DRAFT" environments/porch-demo/work/blueprints/network-function/ --namespace=porch-demo
kpt alpha rpkg propose "$NEW_DRAFT" --namespace=porch-demo
kpt alpha rpkg approve "$NEW_DRAFT" --namespace=porch-demo
```

### Key Points

- Old published revisions (v1, v2) **stay around as history** — you never need to delete them to create new ones.
- You can only `push` to a **Draft** package. Published and Proposed packages are immutable.
- Both `init` and `copy` create server-side packages, so both require `pull` before `push` (see Q19).

### Deleting Package Revisions (if needed)

If you need to clean up stale revisions:
```bash
# Delete a draft
kpt alpha rpkg del <package-name> --namespace=porch-demo

# Force-delete a published package (remove finalizer first)
kubectl patch packagerevision <package-name> -n porch-demo --type=json -p='[{"op":"remove","path":"/metadata/finalizers"}]'
kubectl delete packagerevision <package-name> -n porch-demo
```
